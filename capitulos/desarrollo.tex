...
\begin{itemize}
    ...
    \item \textbf{Interactores y componentes de delección:} Módulos que gestionan las mecánicas de interacción, proveyendo herramientas nativas para la manipulación directa por proximidad y la selección a distancia mediante punteros virtuales (raycast).
...
Para que las herramientas de interacción a distancia (el escáner de mapa de calor o el puntero de selección) operen de forma precisa sobre el modelo arquitectónico, es necesario que cada elemento geométrico del edicio posea un volumen físico independiente que pueda registrar impactos de los punteros. Configurar esto de forma manual en modelos complejos que contienen miles de piezas resulta inviable.
...
Para evaluar discrepancias, el usuario cuenta con una "cinta métrica virtual" capaz de calcular distancias exactas entre cualquier par de puntos del espacio tridimensional a partir del modelo teórico. Visto en la posterior Figura \ref{fig:informe-diseño}.
...
La automatización del informe documental consiste en una arquitectura de memoria centralizada. Todas las herramientas de la aplicación operan independientemente, pero vuelcan su información en estructuras de datos globales. Al solicitar la exportación, un motor basado en librerías de procesamiento de documentos organiza estos datos y lo estructura en un archivo PDF.
...
Esta evaluación se realiza a través de un sistema de clasificación que analiza la magnitud del error detectado y altera en consecuencia la el color de la superficie analizada:
\begin{itemize}
